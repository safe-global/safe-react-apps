...
describe("useAirdropFile", () => {
  it("not existing file / no airdrop", async () => {
    ...
    expect(result.result.current[0]).toEqual([null, null])
  ...
  it("error while fetching data", async () => {
    ...
    expect(result.result.current[0]).toEqual([undefined, undefined])
  ...
  it("user and ecosystem vesting", async () => {
    const testVesting = [
      {
        tag: "user",
        account: mockSafeAddress,
